...
const GameGrid = ({games, handleParentSort}) => {

    const [sortedGames, setSortedGames] = useState([]);

    useEffect(() => {
        handleParentSort(sortedGames);
    }, [sortedGames])
    ...
    const sortGamesAscending = (type) => {
        ...
        const sortProperty = types[type];
        const sorted = [...games].sort((a, b) => a[sortProperty] - b[sortProperty]);
        console.log(sorted);
        setSortedGames(sorted);
    }
    ...
    const handleSort = (event) => {
        ...
        if(sorter === 'minPlayersAsc'|| 'maxPlayersAsc' || 'playTimeAsc' || 'categoryAsc') {
            sortGamesAscending(sorter);
            
        } else if(sorter === 'minPlayersDesc'|| 'maxPlayersDesc' || 'playTimeDesc') { 
            sortGamesDescending(sorter);
        }
    }
            ...
            <Form>
                <Form.Field>
                    <select onChange={handleSort}>
                        <option defaultValue='selected'>All Results</option>
